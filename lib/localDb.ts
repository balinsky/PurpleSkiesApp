...
export type LocalEntry = {
  ...
  fledged_count: number; renesting_attempt: number;
    ...
    await D.runAsync(
      `INSERT OR IGNORE INTO nest_check_entries
       (id,nest_check_id,compartment_id,species,is_empty_cavity,has_nest,nest_discarded,nest_replaced,
        ...
        dead_young_count,dead_adult_male,dead_adult_female,fledged_count,renesting_attempt,
      ...
      [
        ...
        E.fledged_count ?? 0, E.renesting_attempt ? 1 : 0,
...
export async function upsertLocalEntry(E: {
  ...
  fledged_count: number; renesting_attempt: boolean;
  notes: string | null; observed_male_age: string | null; observed_female_age: string | null;
}): Promise<void> {
  const D = await db();
  await D.runAsync(
    ...
    [
      ...
      E.fledged_count, E.renesting_attempt ? 1 : 0,
...
export async function getLocalEntriesForCompartment(
  ...
  check_date: string; species: string; is_empty_cavity: number; has_nest: number;
  ...
  const D = await db();
  return D.getAllAsync(
    `SELECT nce.species, nce.is_empty_cavity, nce.has_nest, nce.egg_count, nce.discarded_eggs, nce.young_count,
            nce.nestling_age_days, nce.observed_male_age, nce.observed_female_age,
            nce.nest_check_id, nc.check_date
